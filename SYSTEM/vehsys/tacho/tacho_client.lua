		...
		if(lastseat==0)then
			...
			if(getPedOccupiedVehicle ( getLocalPlayer()) )then
				...
				if(tx)then
					...
					if(getElementData(vehicle,"muntruck") or getElementData(vehicle,"atomveh") or getElementData(vehicle,"isGTruck"))then--wtruck
						if(messpeed>(1/180*100))then
							local mult=((1/180*100)/(messpeed/100))/100
							setElementVelocity(getPedOccupiedVehicle ( getLocalPlayer()),tx*mult,ty*mult,tz*mult)
							maxspeed=(1/180*100)
						end
					end
					if(getElementModel(getPedOccupiedVehicle ( getLocalPlayer()))==462)then--faggio
						if(messpeed>(1/180*50))then
							local mult=((1/180*50)/(messpeed/100))/100
							setElementVelocity(getPedOccupiedVehicle ( getLocalPlayer()),tx*mult,ty*mult,tz*mult)
							maxspeed=(1/180*50)
						end
					end
					if(getElementModel(getPedOccupiedVehicle ( getLocalPlayer()))==448)then--Pizza
						if(messpeed>(1/180*50))then
							local mult=((1/180*50)/(messpeed/100))/100
							setElementVelocity(getPedOccupiedVehicle ( getLocalPlayer()),tx*mult,ty*mult,tz*mult)
							maxspeed=(1/180*50)
					...
					if(getElementModel(getPedOccupiedVehicle ( getLocalPlayer()))==471)then--quad
						if(messpeed>(1/180*90))then
							local mult=((1/180*90)/(messpeed/100))/100
							setElementVelocity(getPedOccupiedVehicle ( getLocalPlayer()),tx*mult,ty*mult,tz*mult)
							maxspeed=(1/180*90)
						end
					end
