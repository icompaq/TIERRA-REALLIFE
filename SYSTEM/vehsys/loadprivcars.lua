...
function loadPrivCars()
	local privquery="SELECT * FROM vehicles"
	...
	if( not result) then
		...
		while (mysql_num_rows(result)>zahler) do
			...
			if(tonumber(dasatz["SpawnX"])==0 and tonumber(dasatz["SpawnY"])==0 and tonumber(dasatz["SpawnZ"])==0) and not(abgeschleppt==1) then
					...
					mysql_query(handler,"UPDATE vehicles SET abgeschleppt=1 WHERE ID='"..dasatz["ID"].."'")				
...
function save_car(veh)
	...
	local setstring=""
	setstring=setstring.."Tuning='"..vioGetElementData(veh,"tuning").."'"
	setstring=setstring..",Colors='"..vioGetElementData(veh,"colors").."'"
	setstring=setstring..",paintjob='"..vioGetElementData(veh,"paintjob").."'"
	setstring=setstring..",Tank='"..vioGetElementData(veh,"tank").."'"
	setstring=setstring..",kmstand='"..vioGetElementData(veh,"kmstand").."'"
	setstring=setstring..",Lichterfarbe='"..vioGetElementData(veh,"Lichterfarbe").."'"
	setstring=setstring..",premColors='"..vioGetElementData(veh,"premColor").."'"
	local query="UPDATE vehicles SET "..setstring.." WHERE ID='"..vioGetElementData(veh,"dbid").."'"
...
function onvehicleexplode_exec(source)
	...
	if(privCars[source])then		
		if(getPlayerFromName(vioGetElementData(source,"besitzer")))then
			...
			if(dis<50)then
			local query="DELETE FROM vehicles WHERE ID='"..vioGetElementData(source,"dbid").."'"
		...
		elseif(spawnDiscconectPlayers[vioGetElementData(source,"besitzer")])then
			...
			if(versicherung==1 or premium>0)then
				...
				--Ausgabe und Auszahlung	
				local query="INSERT INTO gutschriften (Nickname,Grund,Geld) VALUES ('"..vioGetElementData(source,"besitzer").."','Fahrzeugsystem: Eine Versicherung hat ihnen "..math.round(satz*100).."% vom Einkaufspreis wieder gutgeschrieben!','"..(wert*satz).."')"
				mysql_query(handler,query)
			end			
			destroyElement(source)
			privCars[source]=nil
