...
function admins_func(theMaker,command)
	...
	local adminpre=" "
	for theKey,thePlayer in ipairs(getElementsByType ( "player" )) do -- use a generic for loop to step through each player
        if(isPlayerLoggedIn(thePlayer))then
			if(vioGetElementData(thePlayer,"adminlvl")>0)then
				...
				if(vioGetElementData(thePlayer,"adminlvl")==5) then 
			...
			if(vioGetElementData(thePlayer,"SupportLVL")>0)then
				outputChatBox("Supporter: "..getPlayerName(thePlayer),theMaker)
			end
        end
	end
