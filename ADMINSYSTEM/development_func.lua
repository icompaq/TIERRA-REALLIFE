...
local saveX,saveY,saveZ=0,0,0

function saveandgetdistance_dev_func(thePlayer,cmd,state)
    if(state=="new")then
        local saveX,saveY,saveZ=getElementPosition(thePlayer)
        vioSetElementData(thePlayer,"savePos",{saveX,saveY,saveZ})
    end
    local saves=    vioGetElementData(thePlayer,"savePos")
    outputChatBox(math.round(getElementsDistanceToPoint(thePlayer,saves[1],saves[2],saves[3]),2).." Koordinaten",thePlayer)

end
addCommandHandler("sgd",saveandgetdistance_dev_func,false,false)

function saveandgetarea_dev_func(thePlayer,cmd,state)
    if(state=="new")then
        vioSetElementData(thePlayer,"savePos",{saveX,saveY,saveZ})
    end
    local saves=    vioGetElementData(thePlayer,"savePos")
    outputChatBox(math.round(getElementsAreaToPoint(thePlayer,saves[1],saves[2],saves[3]),2).." QM",thePlayer)

end
addCommandHandler("sga",saveandgetarea_dev_func,false,false)
