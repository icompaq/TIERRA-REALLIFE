...
function showSkinGui_func(duty)
	...
	if(fraktion==0)or(fraktion==8)or(duty)then
		...
		for key, index in pairs ( tabler ) do
			if (not(policeSkin[index]) and not(groveSkin[index]) and not(newsSkin[index]) and not(llsSkin[index]) and not(sekSkin[index]) and not (armySkin[index]) and not(medicSkin[index]) and not(violettasSkin[index]) and not(cashForceSkin[index]))then
