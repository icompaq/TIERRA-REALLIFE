...
function onResourceCreateHotDogMarkerAndVehicles()
	...
	vehicle= createVehicle(588,-57.76953125,-215.279296875,5.3341941833496,358.67065429688,359.99450683594,173.91906738281)    --  hitdog 1 
...
--/sellhotdog [Menge] [Preis] [Spielername]
		...
		if(getPedOccupiedVehicleSeat (thePlayer )==0)then
			if(getElementModel(getPedOccupiedVehicle(thePlayer))==588)then
				...
				if(mengeStr and preisStr and toPlayerNamePart)then
					menge=tonumber(mengeStr)
					preis=tonumber(preisStr)
					if(menge and preis)then
						if(menge<0)then 
							return false
						end
						toPlayer=getPlayerFromIncompleteName(toPlayerNamePart)
						if(toPlayer)then
