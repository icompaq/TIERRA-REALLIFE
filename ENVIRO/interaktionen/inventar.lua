...
function use_func(thePlayer,Command,wasString)
	...
	elseif(wasString=="Rubbellose")then
		if(vioGetElementData(thePlayer,"rubbellos"))then
			if(vioGetElementData(thePlayer,"rubbellos")>0)then
				local numA=math.random(1,10)
				local numB=math.random(1,10)
				local numC=math.random(1,10)
				if(numA==numB and numB==numC)then
					local gewinn=numA*100.00
					outputChatBox(string.format("Du hast die Felder freigerubbelt! Die Zahlen waren: %s %s %s", numA, numB, numC),thePlayer)
					outputChatBox(string.format("Du gewinnst %s", toprice(gewinn)),thePlayer)
                    changePlayerMoney(thePlayer,gewinn,"sonstiges","Rubbellosgewinn")
					if(numC==10)then
						if(vioGetElementData(thePlayer,"Erfolg_10erLos")~=1)then
							vioSetElementData(thePlayer,"Erfolg_10erLos",1)
							triggerClientEvent(thePlayer,"onClientCreatePokalGUI",thePlayer,"10er Los","Gewinne mit drei Zehnen auf dem Rubbellos")
						end			
								
					end
					if(vioGetElementData(thePlayer,"Erfolg_Rubbellosgluck")~=1)then
						vioSetElementData(thePlayer,"Erfolg_Rubbellosgluck",1)
						triggerClientEvent(thePlayer,"onClientCreatePokalGUI",thePlayer,"Rubbellosglueck","Gewinne einmal im Rubbellosspiel")
					end
					
					
				elseif(numA==numB-1 and numB==numC-1)then
					if(numC==10)then numC=0 end
					local gewinn=tonumber(tostring(numA)..tostring(numB)..tostring(numC))/100
					outputChatBox(string.format("Du hast die Felder freigerubbelt! Die Zahlen waren: %s %s %s", numA, numB, numC),thePlayer)
					outputChatBox(string.format("Du gewinnst %s", toprice(gewinn)),thePlayer)
                    changePlayerMoney(thePlayer,gewinn,"sonstiges","Rubbellosgewinn")
					if(vioGetElementData(thePlayer,"Erfolg_Rubbellosgluck")~=1)then
					...
					outputChatBox(string.format("Du hast die Felder freigerubbelt! Die Zahlen waren: %s %s %s", numA, numB, numC),thePlayer)
					outputChatBox("Oh ... Leider kein Gewinn!",thePlayer)
				end
				vioSetElementData(thePlayer,"rubbellos",vioGetElementData(thePlayer,"rubbellos")-1)
			end	
		end
