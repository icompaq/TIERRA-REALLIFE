	...
	if not(vioGetElementData(source,"inArena"))then
		...
		if(attacker) then
			...
			if(isBeamter(attacker)) and (vioGetElementData(source,"wanteds")>0) then
                ...
                changePlayerMoney(source, ((((vioGetElementData(source, "wanteds") * 20) * vioGetElementData(source, "wanteds")) * 10) * -1), "sonstiges", "Knast durch Tod")
				vioSetElementData(source,"knastzeit",vioGetElementData(source,"knastzeit")+(vioGetElementData(source,"wanteds")*10))
