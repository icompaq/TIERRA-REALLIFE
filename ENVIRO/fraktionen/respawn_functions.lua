...
function respawnFrakVehicle(id, admin, thePlayer)
    ...
    if(admin)then
        frakdepot_log(frak,1,-summe,string.format("Fahrzeugrespawn by Admin-%s", getPlayerName(thePlayer)))
    else
        frakdepot_log(frak,1,-summe,string.format("Fahrzeugrespawn by %s", getPlayerName(thePlayer)))
    end
end
    ...
    if(vioGetElementData(thePlayer,"fraktion")>0 and vioGetElementData(thePlayer,"fraktionsrang")>4)then
        ...
        if(frakselfcars[frakID] and frakkasse[frakID])then
            if(frakkasse[frakID]<500)then
                outputChatBox("In der Fraktionskasse befindet sich nicht genügend Geld! (Kosten pro Respawn: 500$",thePlayer,255,0,0)
            else
