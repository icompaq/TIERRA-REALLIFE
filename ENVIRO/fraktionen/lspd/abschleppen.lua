				...
				if(vioGetElementData(thePlayer,"slot"..id)==-2)then
					if(getPlayerMoney(thePlayer)<abholpreis)then
						outputChatBox(string.format("Du hast nicht genügend Geld bei dir! Du benötigst %s$", abholpreis),thePlayer,211,22,0)
							...
							local privquery="SELECT * FROM vehicles WHERE Besitzer='"..getPlayerName(thePlayer).."' and SlotID='"..id.."'"
                            ...
                            --@todo: SET KOORDINATEN 0
							local resultre=mysql_query(handler,"UPDATE vehicles SET abgeschleppt=0 WHERE ID='"..dasatz["ID"].."'")
