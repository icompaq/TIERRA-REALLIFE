...
function controlWeber(thePlayer)
	if math.round(vioGetElementData(thePlayer,"playtime")/60)==25 then
		local werber=MySQL_GetString("players","werber","Nickname='"..getPlayerName(thePlayer).."'")
		if(MySQL_DatasetExist("players","Nickname='"..werber.."'"))then
			if(getPlayerName(thePlayer)~=werber)then
				...
				mysql_query(handler,query)
				local maxwerb=MySQL_GetVar("userdata", "werbernum", "Nickname='"..werber.."'")
				if(maxwerb==5)then
...
function payday(thePlayer)
	...
	if(vioGetElementData(thePlayer,"fraktion")>0)then	
		local additional=MySQL_GetVar("fraktionskasse","gehalt", "FrakID='"..vioGetElementData(thePlayer,"fraktion").."'")
		local bonus=additional
		
		local query= "SELECT count(*) as Anzahl ,userdata.Fraktion,fraktionskasse.max_members FROM `userdata` LEFT JOIN fraktionskasse ON fraktionskasse.FrakID=userdata.Fraktion WHERE userdata.Fraktion<>'0' AND userdata.Fraktion<>'8' AND userdata.Fraktion<>'7' AND userdata.Fraktion<>'5' GROUP BY userdata.FRAKTION"
		...
		local valueFrak=0
		local diff=0
		-- for theKey,theRow in (mysql_rows_assoc ( result )) do
		while true do
			local theRow = mysql_fetch_assoc(result)
			if(not theRow)then break end
			schnitt=schnitt+(tonumber(theRow["Anzahl"])/tonumber(theRow["max_members"]))
			if(tonumber(theRow["Fraktion"])==vioGetElementData(thePlayer,"fraktion"))then
				valueFrak=(tonumber(theRow["Anzahl"])/tonumber(theRow["max_members"]))
				diff=4/tonumber(theRow["max_members"])
			end
			values=values+1
		end
		schnitt=(schnitt/values)-valueFrak
		if(schnitt>diff)then
			bonus=bonus+200
		end	
