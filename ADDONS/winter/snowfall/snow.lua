				...
				--	outputDebugString(string.format("Flake removed. %.1f %.1f %.1f",flake.x,flake.y,flake.z))
					...
					if ground[gx][gy] and (flake.z+lz) > ground[gx][gy] then
						...
						if draw_x and draw_y then
							...
							dxDrawImageSection(draw_x, draw_y, flake.size, flake.size, flake.section[1], flake.section[2], 32, 32, "ADDONS/winter/snowfall/flakes/".. settings.type .. "_tile.png", flake.rot, 0, 0, tocolor(222,235,255,flake.alpha))
							--	end
...
function setSnowValues()
	...
	setRainLevel(0)
	if(int>0)then
		updateSnowDensity((300+(2000*int)),false)
	else
		updateSnowDensity(300,false)	
	end
	updateSnowWindDirection(1,1)
	updateSnowWindSpeed(20)
...
function CheckWindChange()
	local x,y,z=getWindVelocity ( )
	local betrag=math.sqrt(x*x+y*y)
	updateSnowWindDirection((x/betrag),(y/betrag))
	
	updateSnowFallSpeed(1,math.sqrt(z*z)*10+5)
	
	updateSnowWindSpeed(betrag*25)
end
